	...
	<script type="text/javascript"> 
		...
		jQuery(document).ready(function() {   
			try {
				...
				jQuery("#list").jqGrid({
				  caption: '物理主机列表',
				  url: contextPath + '/testing/demo/host.xml', // 'api.do?pageAction=module.config.app.queryAppList', 
				  datatype: 'xml',
				  mtype: 'POST',
				  xmlReader : { 
					  root:"rows", 
				  ...
				  colNames:['虚拟主机ID', '主机名称', '主机IP', '共享模式', '主机类型', '主机状态', '备注'],
				  colModel :[ 
				    {name:'host_id', index:'host_id',hidden:true, editable:true, editrules:{edithidden:true, required:true}, width:120}, 
				    {name:'host_name', index:'host_name', editable:true, sortable:false, width:120}, 
				    {name:'host_ip', index:'host_ip', editable:true, sortable:false, width:120}, 
				    {name:'share_mode', index:'share_mode', editable:true,
					    edittype:'select', formatter:'select', 
					    editoptions:{value:"0:保留;1:独占;2:共享"}, sortable:false,
					    width:120, align:'center'}, 
					{name:'host_type', index:'host_type', editable:true, 
							edittype:'select', formatter:'select', 
						    editoptions:{value:"pc:PC机器;mini_frame:小型机"}, sortable:false, width:120}, 
				    {name:'host_state', index:'host_state', editable:true, 
							    edittype:'select', formatter:'select', 
